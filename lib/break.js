...
export function canShowBreakManagement({
  ...
  if (attendancePolicy && attendancePolicy.allowBreaks === false) return false;
...
/**
 * Normalize break policy + usage from today/shift/attendance-policy payloads.
 * Example company rule: 60 min break allowance, max 3 splits →
 * maxBreaks=3, allowedBreakMinutes=60.
 *
 * Priority: today fields → attendancePolicy API → shift → nested breakPolicy.
 */
export function getBreakPolicy(today, shift, attendancePolicy) {
  const policy =
    today?.breakPolicy ||
    today?.breakSettings ||
    today?.break ||
  ...
  const maxBreaks = pickNumber(
    ...
    policy.breakLimit,
    attendancePolicy?.maxBreaks,
    attendancePolicy?.maxBreakCount,
    shift?.maxBreaks,
    shift?.allowedBreaks,
    shift?.maxBreakCount
  );
  ...
  const allowedBreakMinutes = pickNumber(
    ...
    attendancePolicy?.maxBreakMinutes,
