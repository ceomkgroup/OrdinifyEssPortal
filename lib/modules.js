import { getRequestTypeByHref } from "@/lib/request-types";
import {
  getDocumentTypeByHref,
  getDocumentTypeByKey,
} from "@/lib/document-types";
import { getAssetNavByHref, getAssetNavByKey } from "@/lib/asset-nav";
import { getLeaveNavByHref, getLeaveNavByKey } from "@/lib/leave-nav";
import { getPayslipNavByHref, getPayslipNavByKey } from "@/lib/payslip-nav";
import { getTeamNavByHref } from "@/lib/team-nav";

/**
 * Company modules drive which portal screens / widgets are available.
 *
 * API: GET /api/employee/portal/company-modules
 *  - screenControllers: string[]  → enabled product screens
 *  - feature flags: allowLeaveEncashment, overtimeModule, …
 *
 * ALSO apply employee permissions on top (see lib/permissions.js):
 * e.g. attendance module ON + allowWebPunch false → show status, hide punch actions.
 */

/** Sidebar / route → required screenControllers (any match = show) */
export const ROUTE_MODULES = {
  "/dashboard": ["dashboard"],
  "/profile": null, // always available in ESS
  "/profile/edit": null,
  "/attendance": ["attendance"],
  "/roster": ["shifts", "attendance"],
  "/leave": ["leave"],
  "/leave/logs": ["leave"],
  "/leave/encashment": ["leave"],
  "/leave/balance": ["leave"],
  "/requests": [
    "leave",
    "wfhRequests",
    "onDutyRequests",
    "compOff",
    "overtimeRequests",
    "shiftChangeRequests",
    "attendanceChange",
    "loans",
    "advances",
    "expenseClaims",
  ],
  "/requests/leave": ["leave"],
  "/requests/encashment": ["leave"],
  "/requests/attendance-change": ["attendanceChange"],
  "/requests/shift-change": ["shiftChangeRequests"],
  "/requests/wfh": ["wfhRequests"],
  "/requests/on-duty": ["onDutyRequests"],
  "/requests/overtime": ["overtimeRequests"],
  "/requests/comp-off": ["compOff"],
  "/requests/loans": ["loans"],
  "/requests/advances": ["advances"],
  "/requests/expense-claims": null,
  "/documents": null, // always available in ESS
  "/documents/my": null,
  "/documents/company": null,
  "/assets": null, // always available in ESS
  "/assets/my": null,
  "/assets/verifications": null,
  "/assets/incidents": null,
  "/payslip": ["payroll"],
  "/payslip/tax-certificate": ["payroll"],
  "/team": ["employees"],
  "/team/requests": ["employees"],
  "/team/attendance": ["employees"],
  "/team/corrections": ["employees"],
  "/holidays": ["companyHolidays"],
  "/announcements": null, // always available in ESS (portal announcements)
  "/reports": ["reports"],
  "/settings": ["settings"],
  "/settings/security": ["settings"],
};

/** Dashboard widget visibility */
export const WIDGET_MODULES = {
  leaveBalance: ["leave"],
  attendanceMonth: ["attendance"],
  weeklyHours: ["attendance"],
  pendingRequests: [
    "leave",
    "wfhRequests",
    "onDutyRequests",
    "compOff",
    "overtimeRequests",
    "shiftChangeRequests",
    "loans",
    "advances",
    "expenseClaims",
  ],
  upcomingHolidays: ["companyHolidays"],
  teamMembers: ["employees"],
  lastPayslip: ["payroll"],
  companySettings: ["settings"],
  punchWidget: ["attendance", "dashboard"],
  punchPermissions: ["attendance", "dashboard"],
  todayStatus: ["attendance", "dashboard"],
  shift: ["shifts", "dashboard"],
};

/** Pending-request tile → module / feature flag */
export const REQUEST_TILE_MODULES = {
  leave: { screens: ["leave"] },
  shiftChange: { screens: ["shiftChangeRequests"] },
  wfh: { screens: ["wfhRequests"] },
  onDuty: { screens: ["onDutyRequests"] },
  overtime: { screens: ["overtimeRequests"] },
  compOff: { screens: ["compOff"] },
  loans: { screens: ["loans"] },
  advances: { screens: ["advances"] },
  expenseClaims: { screens: null },
  encashment: { screens: ["leave"], flag: "allowLeaveEncashment" },
  attendanceChange: { screens: ["attendanceChange"] },
};

export function normalizeModules(raw = {}) {
  const screenControllers = Array.isArray(raw.screenControllers)
    ? raw.screenControllers
    : [];

  const features =
    raw.features && typeof raw.features === "object" ? raw.features : {};
  const flags = raw.flags && typeof raw.flags === "object" ? raw.flags : {};

  const truthy = (v) =>
    v === true || v === 1 || v === "1" || v === "true" || v === "yes";

  const allowLeaveEncashment = [
    raw.allowLeaveEncashment,
    raw.leaveEncashment,
    raw.allow_leave_encashment,
    raw.leaveEncashmentEnabled,
    features.allowLeaveEncashment,
    features.leaveEncashment,
    flags.allowLeaveEncashment,
    flags.leaveEncashment,
  ].some(truthy);

  const hasEncashmentScreen = screenControllers.some((s) => {
    const key = String(s || "").toLowerCase();
    return (
      key === "encashment" ||
      key === "leaveencashment" ||
      key === "leave_encashment" ||
      key === "leave-encashment"
    );
  });

  const hasOvertimeScreen = screenControllers.some((s) => {
    const key = String(s || "")
      .toLowerCase()
      .replace(/[_-]/g, "");
    return (
      key === "overtimerequests" ||
      key === "overtimerequest" ||
      key === "overtime"
    );
  });

  const overtimeModule =
    [
      raw.overtimeModule,
      features.overtimeModule,
      flags.overtimeModule,
    ].some(truthy) || hasOvertimeScreen;

  return {
    screenControllers,
    screens: new Set(screenControllers),
    allowLeaveEncashment: allowLeaveEncashment || hasEncashmentScreen,
    breakManagement: Boolean(raw.breakManagement ?? features.breakManagement),
    overtimeModule,
    geofence: Boolean(raw.geofence ?? features.geofence),
    liveTracking: Boolean(raw.liveTracking ?? features.liveTracking),
    penaltiesModule: Boolean(raw.penaltiesModule ?? features.penaltiesModule),
    shiftRulesModule: Boolean(
      raw.shiftRulesModule ?? features.shiftRulesModule
    ),
    halfDayRules: Boolean(raw.halfDayRules ?? features.halfDayRules),
  };
}

export function hasScreen(modules, key) {
  if (!key) return true;
  if (!modules?.screens) return false;
  if (modules.screens.has(key)) return true;
  // Match orgDocuments / org-documents / org_documents variants
  const norm = String(key)
    .toLowerCase()
    .replace(/[_-]/g, "");
  for (const screen of modules.screens) {
    const s = String(screen || "")
      .toLowerCase()
      .replace(/[_-]/g, "");
    if (s === norm) return true;
  }
  return false;
}

export function hasAnyScreen(modules, keys = []) {
  if (!keys?.length) return true;
  return keys.some((key) => hasScreen(modules, key));
}

export function hasFlag(modules, flag) {
  if (!flag) return true;
  return Boolean(modules?.[flag]);
}

export function canAccessRoute(modules, href) {
  // Request sub-routes: screen + optional feature flag
  const requestType = getRequestTypeByHref(href);
  if (requestType) {
    if (requestType.screens == null) {
      if (requestType.flag && modules && !hasFlag(modules, requestType.flag)) {
        return false;
      }
      return true;
    }
    if (!modules) return false;
    if (!hasAnyScreen(modules, requestType.screens)) return false;
    if (requestType.flag && !hasFlag(modules, requestType.flag)) return false;
    return true;
  }

  // Team sub-routes (MSS) — isManager is gated separately
  const teamNav = getTeamNavByHref(href);
  if (teamNav) {
    if (teamNav.screens == null) return true;
    if (!modules) return false;
    return hasAnyScreen(modules, teamNav.screens);
  }
  const payslipNav = getPayslipNavByHref(href);
  if (payslipNav) {
    if (payslipNav.screens == null) return true;
    if (!modules) return false;
    return hasAnyScreen(modules, payslipNav.screens);
  }

  // Leave sub-routes
  const leaveNav = getLeaveNavByHref(href);
  if (leaveNav) {
    if (leaveNav.screens == null) {
      if (leaveNav.flag && modules && !hasFlag(modules, leaveNav.flag)) {
        return false;
      }
      return true;
    }
    if (!modules) return false;
    if (!hasAnyScreen(modules, leaveNav.screens)) return false;
    if (leaveNav.flag && !hasFlag(modules, leaveNav.flag)) return false;
    return true;
  }

  // Documents sub-routes
  const documentType = getDocumentTypeByHref(href);
  if (documentType) {
    if (documentType.screens == null) return true;
    if (!modules) return false;
    return hasAnyScreen(modules, documentType.screens);
  }

  // Assets sub-routes
  const assetNav = getAssetNavByHref(href);
  if (assetNav) {
    if (assetNav.screens == null) return true;
    if (!modules) return false;
    return hasAnyScreen(modules, assetNav.screens);
  }

  if (Object.prototype.hasOwnProperty.call(ROUTE_MODULES, href)) {
    const required = ROUTE_MODULES[href];
    if (required == null) return true;
    if (!modules) return href === "/dashboard" || href === "/profile";
    return hasAnyScreen(modules, required);
  }

  if (href?.startsWith("/requests/")) {
    if (!modules) return false;
    return canAccessRoute(modules, "/requests");
  }

  if (href?.startsWith("/leave/")) {
    if (!modules) return false;
    return canAccessRoute(modules, "/leave");
  }

  if (href?.startsWith("/team/")) {
    if (!modules) return false;
    return canAccessRoute(modules, "/team");
  }

  if (href?.startsWith("/documents/")) {
    if (!modules) return false;
    return canAccessRoute(modules, "/documents");
  }

  if (href?.startsWith("/assets/")) {
    return true;
  }

  if (href?.startsWith("/profile/")) {
    return true;
  }

  if (href?.startsWith("/settings/")) {
    return canAccessRoute(modules, "/settings");
  }

  if (!modules) return href === "/dashboard" || href === "/profile";
  return true;
}

export function canShowPayslipTile(modules, tileKey) {
  const type = getPayslipNavByKey(tileKey);
  if (!type) return true;
  if (type.screens == null) return true;
  if (!modules) return false;
  return hasAnyScreen(modules, type.screens);
}

export function canShowLeaveTile(modules, tileKey) {
  const type = getLeaveNavByKey(tileKey);
  if (!type) return true;
  if (type.screens == null) {
    if (type.flag && modules && !hasFlag(modules, type.flag)) return false;
    return true;
  }
  if (!modules) return false;
  if (!hasAnyScreen(modules, type.screens)) return false;
  if (type.flag && !hasFlag(modules, type.flag)) return false;
  return true;
}

export function canShowDocumentTile(modules, tileKey) {
  const type = getDocumentTypeByKey(tileKey);
  if (!type) return true;
  // null screens = always available (ESS documents)
  if (type.screens == null) return true;
  if (!modules) return false;
  return hasAnyScreen(modules, type.screens);
}

export function canShowAssetTile(modules, tileKey) {
  const type = getAssetNavByKey(tileKey);
  if (!type) return true;
  if (type.screens == null) return true;
  if (!modules) return false;
  return hasAnyScreen(modules, type.screens);
}

export function canShowWidget(modules, widgetKey) {
  const required = WIDGET_MODULES[widgetKey];
  if (!required) return true;
  if (!modules) return false;
  return hasAnyScreen(modules, required);
}

export function canShowRequestTile(modules, tileKey) {
  const rule = REQUEST_TILE_MODULES[tileKey];
  if (!rule) return true;
  if (rule.screens == null) {
    if (rule.flag && modules && !hasFlag(modules, rule.flag)) return false;
    return true;
  }
  if (!modules) return false;
  if (!hasAnyScreen(modules, rule.screens)) return false;
  if (rule.flag && !hasFlag(modules, rule.flag)) return false;
  return true;
}
