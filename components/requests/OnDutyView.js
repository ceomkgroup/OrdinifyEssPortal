...
export function OnDutyView({ timeFormat = "12h", dateFormat = "DD/MM/YYYY" }) {
  ...
  return (
    <PortalPage
      fill
      title="On Duty"
      subtitle="Submit and track on-duty / field-visit requests"
      actions={
        <>
          <Button
            type="button"
      ...
      <TablePanel
        ...
        drawerFields={
          <FilterDrawerDateRange
            ...
            hint="Apply uses these dates for on-duty logs."
          />
        }
        onApplyFilters={() => {
        ...
        columns={columns}
        rows={filteredRows}
        getRowKey={(row) => row.onDutyId}
        minWidth="900px"
        loading={loading}
        loadingLabel="Loading requests"
        loadingHint="Fetching On Duty requests…"
        error={error}
        emptyIcon={Inbox}
        emptyTitle={`No ${status === "all" ? "" : `${status} `}requests`}
        emptyHint="Submit a request when you need an on-duty or field visit."
        emptyAction={
          <Button
            type="button"
            className="h-10 rounded-xl"
