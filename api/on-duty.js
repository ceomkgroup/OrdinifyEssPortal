...
export function normalizeOnDutyRow(row) {
  if (!row || typeof row !== "object") return null;
  const onDutyId = pick(row.onDutyId, row.requestId, row.id);
  return {
    ...row,
    onDutyId,
    requestId: onDutyId,
...
/**
 * Own on-duty requests.
 * GET /api/employee/portal/on-duty?status=&page=&limit=
  ...
  try {
    if (status === "cancelled") {
      ...
      const [cancelledRes, rejectedRes] = await Promise.all([
        api.get("/api/employee/portal/on-duty", {
      ...
      const cancelledBody = unwrap(
        cancelledRes.data,
        "Failed to load on-duty requests"
      );
      ...
      for (const row of [
        ...
        const normalized = normalizeOnDutyRow(row);
        if (!normalized) continue;
        const id = normalized.onDutyId || JSON.stringify(normalized);
    ...
    const { data } = await api.get("/api/employee/portal/on-duty", {
      params,
    });
    const body = unwrap(data, "Failed to load on-duty requests");
    const rows = (Array.isArray(body.data) ? body.data : [])
      .map(normalizeOnDutyRow)
      .filter(Boolean);
    ...
    throw toApiError(err, "Failed to load on-duty requests");
  }
}

/**
 * Submit an on-duty / field-visit request.
 * POST /api/employee/portal/on-duty
 * Body: { fromDate, toDate, location, purpose }
 */
export async function createOnDutyRequest(payload) {
  try {
    const { data } = await api.post("/api/employee/portal/on-duty", payload);
    return unwrap(data, "Failed to submit on-duty request");
  } catch (err) {
    throw toApiError(err, "Failed to submit on-duty request");
  }
}

/**
 * Cancel own pending on-duty request.
 * PATCH /api/employee/portal/on-duty/{requestId}/cancel
 */
export async function cancelOnDutyRequest(requestId) {
  const id =
    requestId?.onDutyId ||
    requestId?.requestId ||
    requestId?.id ||
    requestId;
  if (!id) {
    const err = new Error("Missing on-duty request id.");
    err.code = "MISSING_ID";
    throw err;
  }
  try {
    const { data } = await api.patch(
      `/api/employee/portal/on-duty/${id}/cancel`
    );
