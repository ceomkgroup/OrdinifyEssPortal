...
export function useWfhList({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await listWfhRequests({ status, page, limit });
        if (!alive) return;
        setRows(res.rows);
...
export function useWfhStats({ enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await listWfhRequests({
          status: "all",
          page: 1,
          limit: 100,
        });
...
export async function submitWfh(payload) {
  return createWfhRequest(payload);
}

export async function cancelWfh(requestId) {
  return cancelWfhRequest(requestId);
}
